...
const PaymentModal = ({ isOpen, onClose, registration, program }) => {
  ...
  // Use refs for intervals so they don't cause stale closure issues
  const pollingRef = useRef(null);
  const timeRef = useRef(null);
  const timeoutRef = useRef(null);
  const isMountedRef = useRef(true);
  // Track step in ref to avoid stale closure in polling callback
  ...
  const checkPaymentStatus = useCallback(async (checkoutId) => {
    try {
      ...
      if (data.success && data.payment) {
        ...
        if (status === 'completed') {
          ...
          stopPolling();
          setStep('failed');
          setError(data.payment.result_desc || 'Payment was not completed.');
        }
      }
    } catch {
  ...
  const startPolling = useCallback((checkoutId) => {
    ...
    timeRef.current = setInterval(() => {
      ...
      setElapsedTime((prev) => {
        const next = prev + 1;
        if (next === 120) setShowStillWaiting(true);
        return next;
      });
    }, 1000);
    ...
    timeoutRef.current = setTimeout(() => {
      ...
      if (stepRef.current === 'processing') {
        setStep('failed');
        setError(
          'Payment is taking longer than expected. If you completed the payment, it will be processed shortly. Contact us if you need help.'
        );
      }
    }, 300_000);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div
        className="relative bg-white w-full max-w-md rounded-lg shadow-2xl"
        role="dialog"
        aria-modal="true"
        ...
        <div className="p-8">
          ...
          {step === 'phone' && (
            <>
              <div className="text-center mb-6">
                ...
                <h2 id="payment-modal-title" className="text-2xl font-light text-neutral-900 mb-2">
                  Pay with M-Pesa
                </h2>
                <p className="text-neutral-600 font-light text-sm">{program?.title}</p>
          ...
          {step === 'processing' && (
            <div className="text-center py-8">
              ...
              <h2 id="payment-modal-title" className="text-2xl font-light text-neutral-900 mb-4">
                Check Your Phone
              </h2>
              ...
              {showStillWaiting && (
                <div className="bg-amber-50 border border-amber-200 text-amber-800 text-sm p-3 rounded mb-4 flex items-start gap-2 text-left">
          ...
          {step === 'success' && (
            <div className="text-center py-8">
              ...
              <div className="bg-green-50 border border-green-200 p-4 rounded mb-6 text-sm text-left">
                <p className="text-neutral-700 font-medium mb-2">Next Steps:</p>
