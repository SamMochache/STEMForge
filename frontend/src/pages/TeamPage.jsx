...
const SAMPLE_INSTRUCTORS = [
  ...
  },
  {
    ...
    bio: 'Serial roboticist and competition winner. Led 5 teams to international robotics championships. Former aerospace engineer.',
    photo: null,
  ...
  <div className="group bg-white rounded-xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-300 h-full flex flex-col">
    ...
    <div className="relative overflow-hidden bg-gradient-to-br from-blue-100 to-purple-100 aspect-square">
      {instructor.photo ? (
        ...
        <div className="w-full h-full flex items-center justify-center">
          <Users size={64} className="text-neutral-300" aria-hidden="true" />
        </div>
      )}
        ...
        <div className="flex gap-2">
          {instructor.email && (
            <a
              ...
              className="w-10 h-10 bg-white/25 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-white/40 transition-all"
              aria-label={`Email ${instructor.full_name}`}
            >
              <Mail size={18} className="text-white" aria-hidden="true" />
            </a>
          )}
          <a
            href={`https://linkedin.com/search/results/people/?keywords=${encodeURIComponent(instructor.full_name)}`}
            target="_blank"
            rel="noopener noreferrer"
            className="w-10 h-10 bg-white/25 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-white/40 transition-all"
            aria-label={`${instructor.full_name} on LinkedIn`}
          >
    ...
    <div className="p-6 flex-1 flex flex-col">
      <div className="mb-3">
        ...
        {instructor.title && (
          <p className="text-sm font-medium text-blue-600 tracking-wide mt-1">{instructor.title}</p>
        )}
      </div>
      ...
      {(instructor.experience || instructor.education) && (
        <div className="grid grid-cols-2 gap-3 mb-4 pb-4 border-t border-neutral-100 pt-4">
          {instructor.experience && (
            <div>
              <p className="text-xs text-neutral-400 tracking-widest uppercase font-semibold mb-1">
                Experience
              </p>
              <p className="text-sm font-medium text-neutral-900 flex items-center gap-1">
                <Briefcase size={14} className="text-blue-600 flex-shrink-0" aria-hidden="true" />
                {instructor.experience}
              </p>
            </div>
          )}
          {instructor.education && (
            <div>
              ...
              <p className="text-sm font-medium text-neutral-900 flex items-center gap-1">
                <GraduationCap size={14} className="text-blue-600 flex-shrink-0" aria-hidden="true" />
      ...
      {instructor.expertise && instructor.expertise.length > 0 && (
        <div className="mb-4">
          <p className="text-xs text-neutral-400 tracking-widest uppercase font-semibold mb-2">
            Expertise
          </p>
...
const TeamPage = () => {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((data) => {
        ...
        setInstructors(results.length > 0 ? results : SAMPLE_INSTRUCTORS);
      })
      .catch(() => {
        if (!cancelled) setInstructors(SAMPLE_INSTRUCTORS);
      })
      .finally(() => {
  ...
  const displayInstructors = loading ? SAMPLE_INSTRUCTORS : instructors;
      ...
      <section className="pt-40 pb-20 bg-gradient-to-b from-neutral-900 to-neutral-800 text-white px-8">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-xl text-neutral-300 font-light max-w-2xl leading-relaxed">
            Our team of elite educators and industry leaders bring decades of combined experience.
            From Google engineers to competition-winning roboticists, each instructor is dedicated
            to your success.
          </p>
        </div>
      </section>
      ...
      <section className="py-16 px-8 bg-neutral-50 border-b border-neutral-200">
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-4 gap-8">
            {[
              { icon: Award, title: 'Industry Leaders', desc: 'Not just teachers—active practitioners in their fields' },
              { icon: GraduationCap, title: 'Top Credentials', desc: 'PhDs, advanced degrees from top global universities' },
              { icon: Users, title: 'Mentors First', desc: 'Personal attention to help you reach your unique potential' },
              { icon: Book, title: 'Lifelong Learners', desc: 'Continuously updating skills with latest technologies' },
            ].map((item, i) => {
              const Icon = item.icon;
              return (
                <div key={i} className="text-center">
                  <Icon size={32} className="text-blue-600 mx-auto mb-3" aria-hidden="true" />
        ...
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-neutral-600 font-light text-lg max-w-2xl mx-auto">
              Beyond our core team, we partner with industry leaders and university professors who
              guide curriculum and provide mentorship.
            </p>
          </div>
      ...
      <section className="py-20 px-8 bg-gradient-to-r from-neutral-900 to-neutral-800 text-white">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <p className="text-neutral-300 font-light text-lg mb-12 max-w-2xl mx-auto">
            We're always looking for talented educators and professionals who want to make a
            difference in STEM education.
          </p>
          <a
            href="/careers"
